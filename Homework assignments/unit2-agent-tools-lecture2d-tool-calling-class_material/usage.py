...
def _uget(obj, path, default=0):
    """
    Safely fetch nested values from either:
      - dict-style objects: {"input_tokens": ..., "input_tokens_details": {"cached_tokens": ...}}
      - attribute-style objects: usage.input_tokens, usage.input_tokens_details.cached_tokens
    """
    cur = obj
...
def _aggregate_usage(usages):
    ...
    for usage in usages:
        total['input'] += int(_uget(usage, "input_tokens", 0))
        total['cached'] += int(_uget(usage, "input_tokens_details.cached_tokens", 0))
        total['output'] += int(_uget(usage, "output_tokens", 0))
        total['reasoning'] += int(_uget(usage, "output_tokens_details.reasoning_tokens", 0))
    return total
