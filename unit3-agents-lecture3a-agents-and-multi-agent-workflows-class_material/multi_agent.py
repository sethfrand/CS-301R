...
async def main(agent_config: Path):
    ...
    opener = await run_agent(
        ...
        "Start by asking the user where they would like to travel, how many people are going and how long they would like to stay."
    ...
    expander_raw = await run_agent(
        client, toolbox, agents['trip_researcher'],
        expander_input, [], usage
    )
    expander = _parse_json("trip_researcher", expander_raw)
    topic_summary = expander.get("trip_summary", "").strip()
...
if __name__ == '__main__':
    config_file = sys.argv[1] if len(sys.argv) > 1 else 'multi_agent.yaml'
